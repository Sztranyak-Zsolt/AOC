...
from GENERICS.aoc_grid import Position2D, add_positions, mh_distance


class CWire:
    def __init__(self):
        self.position_dict: dict[Position2D, int] = {}

    def create_wire_from_raw(self, p_raw_dir: list[str]):
        dir_dict = {'R': Position2D(-1, 0), 'L': Position2D(1, 0), 'U': Position2D(0, 1), 'D': Position2D(0, -1)}
        ...
        for act_step in p_raw_dir:
            ...
            for step in range(1, int(act_step[1:]) + 1):
                step_counter += 1
                act_position = add_positions(act_position, act_dir)
                if act_position not in self.position_dict:
                    self.position_dict[act_position] = step_counter


def wire_crosses(p_wire1: CWire, p_wire2: CWire) -> dict[Position2D, int]:
    rd = {}
...
def solve_puzzle(p_input_file_path: str) -> (int | str, int | str | None):
    ...
    answer1 = min([mh_distance(Position2D(0, 0), p) for p in crosses])

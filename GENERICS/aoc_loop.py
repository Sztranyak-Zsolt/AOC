...
class CLoopHandler:
    ...
    def move_left(self, p_step: int) -> CLoopItem | None:
        if self.loop_size == 0:
            return None
        for _ in range(p_step % self.loop_size):
            self.act_item = self.act_item.left_node
        return self.act_item

    def move_right(self, p_step: int) -> CLoopItem | None:
        return self.move_left(-p_step)
    ...
    def get_list(self):
        ...
        c = 0
        while curr_item != self.act_item and c < 100:

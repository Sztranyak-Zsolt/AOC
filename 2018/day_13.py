...
class CGridX(CGridBase):
    direction_list = [Position2D(1, 0), Position2D(0, 1), Position2D(-1, 0), Position2D(0, -1)]
    turn_dict = {(0, "\\"): 1, (0, "/"): 3,
                 (2, "\\"): 3, (2, "/"): 1,
                 (1, "\\"): 0, (1, "/"): 2,
                 (3, "\\"): 2, (3, "/"): 0}
    ...
    def __init__(self):
        super().__init__()
        self.chart_dict: dict[Position2D, ChartState] = {}

    def move_charts(self) -> list[Position2D]:

        act_chart_dict = self.chart_dict.copy()
        crash_list: list[Position2D] = []
        ...
        for act_position, act_chart_state in sorted(act_chart_dict.items(), key=lambda k: (k[0][1], k[0][0])):
            ...
            new_position = add_positions(act_position, self.direction_list[act_chart_state.act_direction])
        ...
        for y in range(self.max_y, self.min_y - 1, -1)[yr]:
            ...
            for x in range(self.min_x, self.max_x + 1):
                if (x, y) in self.chart_dict:
                    act_row += f"{'>v<^'[self.chart_dict[(x, y)][0]]}@"[0] * c_length
    ...
    for inp_row in yield_input_data(p_input_file_path, p_whole_row=True):
        ...
        if set('>^<v') & set(inp_row):
            for x, ch in enumerate(inp_row):
                if ch in '>^<v':
                    g.chart_dict[Position2D(x, g.max_y)] = ChartState('>v<^'.index(ch), -1)
